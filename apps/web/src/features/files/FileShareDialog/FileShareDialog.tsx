...
export function FileShareDialog({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') setAccessDropdownOpen(null)
    }
        ...
        <div style={{
          ...
          <div>
            <h3 style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)', margin: 0 }}>
              {isFolder ? t('files.shareDialog.shareFolder') : t('files.shareDialog.shareFile')}
            </h3>
        ...
        <div style={{ padding: '16px 20px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', display: 'block', marginBottom: 6 }}>
              ...
              {t('files.shareDialog.invitePeople')}
            </label>
            <div style={{ display: 'flex', gap: 6 }}>
              ...
              <Button variant="primary" size="sm" onClick={handleInvite}>
                {inviteFeedback ? t('files.shareDialog.invited') : t('files.shareDialog.invite')}
              </Button>
            </div>
            {inviteFeedback && (
          ...
          {sharedWith.length > 0 && (
            <div>
              <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', display: 'block', marginBottom: 8 }}>
                ...
                {t('files.shareDialog.sharedWith')} ({sharedWith.length})
              </label>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', display: 'flex', alignItems: 'center', gap: 4 }}>
                <Globe size={12} />
                {t('files.shareDialog.publicLink')}
              </label>
              <button
            ...
            {publicEnabled && !publicLink && (
              <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{t('files.shareDialog.generatingLink')}</span>
            )}
            {!publicEnabled && (
              <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                {t('files.shareDialog.publicLinkHint')}
              </span>
            )}
          </div>
        </div>
