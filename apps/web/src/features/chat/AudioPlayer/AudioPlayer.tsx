...
/**
 * Resolve a stored audio URL to one the browser can fetch. New uploads are
 * served under /api/uploads; legacy messages stored a bare /uploads path that
 * is not proxied to the API in dev or prod, so rewrite those to /api/uploads.
 */
...
export function AudioPlayer({ url, duration, waveform, onPlayStateChange }: AudioPlayerProps) {
  ...
  useEffect(() => {
    ...
    const onLoaded = () => {
      ...
      // MediaRecorder webm blobs ship without duration metadata, so Chrome
      // reports audio.duration === Infinity until forced to scan to the end.
      // That breaks seeking (currentTime = ratio * Infinity) and the progress
      // bar. Seek far past the end once to make the browser compute the real
      // duration, then reset to the start.
      const fixDuration = () => {
