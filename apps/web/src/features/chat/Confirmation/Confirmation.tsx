...
export function Confirmation({
  ...
  useEffect(() => {
    ...
    function handleKeyDown(e: KeyboardEvent) {
      if (e.key === 'Enter' && onApproveRef.current) {
        onApproveRef.current()
      } else if (e.key === 'Escape' && onRejectRef.current) {
        onRejectRef.current()
      }
    }
  ...
  return (
    <div
      role="region"
      aria-label={t('confirmation.regionLabel')}
      style={{
        padding: '12px 14px',
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: description || state === 'requested' ? 6 : 0 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {state === 'pending' && <Badge variant="neutral" size="sm">{t('confirmation.pending')}</Badge>}
          </div>
        </div>
      </div>
        ...
        <>
          <div style={{ display: 'flex', gap: 8 }}>
            <Button variant="primary" size="sm" onClick={onApprove}>
              {approveLabel ?? t('confirmation.approve')}
            </Button>
            <Button variant="secondary" size="sm" onClick={onReject}>
              {rejectLabel ?? t('confirmation.reject')}
            </Button>
          </div>
          {onApprove && (
            <div style={{ marginTop: 8, fontSize: 11, color: 'var(--text-muted)' }}>
              {t('confirmation.keyboardHint')}
            </div>
          )}
        </>
      )}
