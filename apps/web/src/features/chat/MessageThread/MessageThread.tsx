...
import { ActionCard } from '../../tasks/ActionCard/ActionCard'
import { DateSeparator } from '../../../shared/ui/DateSeparator/DateSeparator'
import { Reasoning } from '../Reasoning/Reasoning'
import { ChainOfThought, type ChainOfThoughtStep } from '../ChainOfThought/ChainOfThought'
import { Sources, type Source } from '../Sources/Sources'
import { PlanCard, type PlanStep } from '../../tasks/PlanCard/PlanCard'
import { QueueList, type QueueSection } from '../../tasks/QueueList/QueueList'
import { InlineTask, type InlineTaskFile } from '../../tasks/InlineTask/InlineTask'
import { Confirmation } from '../Confirmation/Confirmation'
import { ReminderCard } from '../../tasks/ReminderCard/ReminderCard'
import { Suggestion } from '../Suggestion/Suggestion'
import { Attachment } from '../../files/Attachment/Attachment'
import { ReactionBadge } from '../ReactionBadge/ReactionBadge'
import { ReactionBar } from '../ReactionBar/ReactionBar'
import { ForwardSelectionBar } from '../../mail/ForwardSelectionBar/ForwardSelectionBar'
import { DeleteSelectionBar } from '../../workspace/DeleteSelectionBar/DeleteSelectionBar'
import { ForwardModal } from '../../mail/ForwardModal/ForwardModal'
...
export interface ThreadMessage {
  ...
  /** True when authored by the current user (right side). Falls back to role when undefined. */
  isOwn?: boolean
  ...
  attachments?: Array<{
    ...
    fileType: string
    thumbnailUrl?: string
    /** When set, attachment is an image and renders inline (WhatsApp-style). */
    imageUrl?: string
  }>
  confirmation?: {
    title: string
  ...
  /** Interactive in-chat reminder. Card reads live task state by id. */
  reminder?: {
    taskId: string
    title: string
  }
...
export interface MessageThreadProps {
  ...
  onSend?: (value: string, replyTo?: { id: string; author: string; content: string }) => void
  currentUser?: string
  placeholder?: string
  isTyping?: boolean
  showAuthor?: boolean
...
// "Own" = current user's message (right side). DM messages from other humans are
// also role 'user', so alignment must come from isOwn, not role.
const isOwnMessage = (m: ThreadMessage) => m.isOwn ?? m.role === 'user'
...
export function MessageThread({
  ...
  const handleSend = (value: string) => {
    ...
    const msg: ThreadMessage = {
      ...
      readStatus: 'sent',
      replyTo: reply ? { author: reply.author, content: reply.content } : undefined,
    }
    setLocalMessages((prev) => [...prev, msg])
    onSend?.(value, reply ? { id: reply.id, author: reply.author, content: reply.content } : undefined)
    setReplyingTo(null)
  }
  ...
  const toggleMessageSelection = (msg: ThreadMessage) => {
    ...
    if (selectionMode === 'delete' && (!isOwnMessage(msg) || msg.deleted)) return
  ...
  const renderMessages = () => {
    ...
    for (let i = 0; i < localMessages.length; i++) {
      ...
      // In delete mode, only non-deleted own messages are selectable
      const isSelectable = selectionMode === 'forward'
        || (selectionMode === 'delete' && isOwnMessage(msg) && !msg.deleted)
              ...
              {showReactionBar && (
                <div
                  style={{
                    ...
                    right: isOwnMessage(msg) ? 0 : undefined,
                    left: isOwnMessage(msg) ? undefined : 0,
                    zIndex: 2,
                  }}
                >
        ...
        >
          <div
            style={{
              ...
              padding: '6px 10px',
              maxWidth: '65%',
              borderRadius: isOwnMessage(msg) ? '12px 12px 4px 12px' : '12px 12px 12px 4px',
    ...
    <>
      ...
      {msg.toolExecution ? (
        ...
        <MessageBubble
          ...
          showAuthor={showAuthor && !isOwnMessage(msg)}
      ...
      {msg.attachments && msg.attachments.length > 0 && (
        <div style={{ marginTop: 6, display: 'flex', justifyContent: isOwnMessage(msg) ? 'flex-end' : 'flex-start' }}>
        <div style={{ maxWidth: '65%', display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: isOwnMessage(msg) ? 'flex-end' : 'flex-start' }}>
            ...
            if (tool.state === 'approval-requested') {
              return (
                <Confirmation
                  key={tool.id}
                  state="requested"
                  title={t('messageThread.executeTool', { toolName: tool.toolName })}
      ...
      {/* ReminderCard (below) — gated to assignees; hidden otherwise */}
      {msg.reminder && (
        <div style={{ marginTop: 8, maxWidth: '65%' }}>
          <ReminderCard taskId={msg.reminder.taskId} title={msg.reminder.title} />
        </div>
      )}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      <ScrollArea.Root style={{ flex: 1, overflow: 'hidden', background: 'var(--surface-2)' }}>
        <ScrollArea.Viewport ref={viewportRef} style={{ height: '100%' }}>
          {localMessages.length === 0 && !isTyping ? (
            <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>
              {t('messageThread.emptyState')}
            </div>
          ) : (
            <div
              role="log"
              aria-live="polite"
              aria-label={t('messageThread.conversationLog')}
      ...
      {msgMenu && (
        ...
        >
          {getMenuItems(msgMenu.messageId).map(({ label, icon, action, danger }, idx) => {
            if (label === 'separator') {
              return <div key={`sep-${idx}`} style={{ height: 1, background: 'var(--border)', margin: '4px 0' }} />
            }
            return (
              <button
                key={`${label}-${idx}`}
