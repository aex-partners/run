...
import { Users, Puzzle, Building2, ChevronRight, Bot, Sparkles, Wrench, Plus, Search } from 'lucide-react'
import * as ScrollArea from '@radix-ui/react-scroll-area'
import * as Dialog from '@radix-ui/react-dialog'
import { X } from 'lucide-react'
import { UserTable, type User } from '../UserTable/UserTable'
import { PluginCard, type PluginCardProps } from '../PluginCard/PluginCard'
import { AgentCard } from '../AgentCard/AgentCard'
import { SkillCard } from '../SkillCard/SkillCard'
import { AgentForm, type AgentFormData } from '../AgentForm/AgentForm'
import { SkillForm, type SkillFormData } from '../SkillForm/SkillForm'
import { Button } from '../../../shared/ui/Button/Button'
// PluginConfigDialog replaced by PluginConnectDialog (rendered in SettingsPage)
import type { MultiSelectOption } from '../../../shared/ui/MultiSelect/MultiSelect'

export type SettingsSection = 'users' | 'plugins' | 'company' | 'agents' | 'skills' | 'tools'
...
const COMPANY_FIELDS: (keyof CompanyInfo)[] = [
  'name', 'tradeName', 'cnpj', 'address', 'phone', 'email', 'plan', 'activeUsers',
]

const COMPANY_FIELD_LABEL_KEYS: Record<keyof CompanyInfo, string> = {
  name: 'settings.companyFields.legalName',
  tradeName: 'settings.companyFields.tradeName',
  cnpj: 'settings.companyFields.taxId',
  address: 'settings.companyFields.address',
  phone: 'settings.companyFields.phone',
  email: 'settings.companyFields.email',
  plan: 'settings.companyFields.plan',
  activeUsers: 'settings.companyFields.activeUsers',
}
...
export function SettingsScreen(props: SettingsScreenProps) {
  ...
  // Dialog states
  const [dialogType, setDialogType] = useState<'agent' | 'skill' | null>(null)
  ...
  // Dialog open helpers
  const openCreate = (type: 'agent' | 'skill') => {
    setEditId(null)
    setDialogType(type)
  }

  const openEdit = (type: 'agent' | 'skill', id: string) => {
    setEditId(id)
    setDialogType(type)
  }
  ...
  const renderContent = () => {
    switch (activeSection) {
      ...
      case 'tools':
        return (
          <div style={{ padding: '24px' }}>
            <SectionHeader title={t('settings.tools')} subtitle={t('settings.toolsSubtitle')} newLabel={t('new')} />
          ...
          <div style={{ padding: '24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {COMPANY_FIELDS.map((field) => {
                const label = t(COMPANY_FIELD_LABEL_KEYS[field])
                ...
                return (
                  ...
                  >
                    ...
                    {isEditing ? (
                      <input
                        autoFocus
                        aria-label={t('settings.editFieldAria', { field: label })}
                        value={editingValue}
