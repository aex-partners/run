...
export type TriggerType = "EMPTY" | "PIECE" | "SCHEDULE" | "WEBHOOK";
...
export interface FlowBuilderActions {
  ...
  addBranchStep: (routerName: string, branchIndex: number, step: FlowAction) => void;
  addLoopStep: (loopName: string, step: FlowAction) => void;
  ...
  updateStepType: (stepName: string, type: ActionType) => void;
...
export const useFlowBuilderStore = create<FlowBuilderState & FlowBuilderActions>()((set, get) => ({
  ...
  addBranchStep: (routerName, branchIndex, step) => {
    const { flowVersion } = get();
    if (!flowVersion) return;

    const trigger = cloneTrigger(flowVersion.trigger);
    const found = findStep(trigger, routerName);
    if (!found) return;
    const router = found.step as FlowAction;
    if (router.type !== "ROUTER") return;

    if (!router.children) router.children = [];
    // pad slots so index is addressable
    while (router.children.length <= branchIndex) router.children.push(undefined);

    const head = router.children[branchIndex];
    if (!head) {
      router.children[branchIndex] = step;
    } else {
      // append to the tail of this branch's chain
      let tail = head;
      while (tail.nextAction) tail = tail.nextAction;
      tail.nextAction = step;
    }
  ...
  addLoopStep: (loopName, step) => {
    const { flowVersion } = get();
    if (!flowVersion) return;

    const trigger = cloneTrigger(flowVersion.trigger);
    const found = findStep(trigger, loopName);
    if (!found) return;
    const loop = found.step as FlowAction;
    if (loop.type !== "LOOP_ON_ITEMS") return;

    const head = loop.firstLoopAction;
    if (!head) {
      loop.firstLoopAction = step;
    } else {
      let tail = head;
      while (tail.nextAction) tail = tail.nextAction;
      tail.nextAction = step;
  ...
  deleteStep: (stepName) => {
    ...
    const parent = found.parent as unknown as Record<string, unknown>;
  ...
  updateStepType: (stepName, type) => {
    ...
    (found.step as FlowAction).type = type;
    set({ flowVersion: { ...flowVersion, trigger }, dirty: true });
  },
