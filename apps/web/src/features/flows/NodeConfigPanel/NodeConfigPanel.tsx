...
import { JsonEditor } from '../../../shared/ui/JsonEditor/JsonEditor'
...
export function NodeConfigPanel({
  ...
  // Reset config when initialData changes (different node selected)
  React.useEffect(() => {
    setConfig({
      taskType: initialData?.taskType ?? 'inference',
      toolName: initialData?.toolName ?? '',
      ...
      >
        ...
        {onClose && (
          <button
            ...
            aria-label={t('workflows.closePanel')}
          >
            <X size={16} />
          </button>
        )}
      ...
      <div style={{ flex: 1, padding: 16, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {(['inference', 'structured'] as const).map((tt) => (
              ...
              >
                {tt === 'inference' ? t('taskType.inference') : t('taskType.structured')}
              </button>
            ))}
          </div>
        </div>
        ...
        {config.taskType === 'structured' && (
          <div>
            ...
            >
              <option value="">{t('workflows.selectTool')}</option>
              {toolOptions.map((opt) => (
