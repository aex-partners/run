...
const statusConfig: Record<FlowRun["status"], { icon: typeof CheckCircle2; color: string; bg: string; labelKey: string }> = {
  succeeded: { icon: CheckCircle2, color: "#16a34a", bg: "#f0fdf4", labelKey: "flowRunHistory.succeeded" },
  failed: { icon: XCircle, color: "#dc2626", bg: "#fef2f2", labelKey: "status.failed" },
  running: { icon: Loader2, color: "#ea580c", bg: "#fff7ed", labelKey: "status.running" },
  paused: { icon: PauseCircle, color: "#ca8a04", bg: "#fefce8", labelKey: "status.paused" },
  stopped: { icon: StopCircle, color: "#6b7280", bg: "#f9fafb", labelKey: "flowRunHistory.stopped" },
};
...
export function FlowRunHistory({ runs, loading }: FlowRunHistoryProps) {
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text)" }}>{t("flowRunHistory.title")}</span>
        <button
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "8px 0" }}>
        {loading && (
          <div style={{ padding: "24px 16px", textAlign: "center", color: "var(--text-muted)", fontSize: 12 }}>
            ...
            {t("flowRunHistory.loading")}
          </div>
        )}

        {!loading && runs.length === 0 && (
          <div style={{ padding: "24px 16px", textAlign: "center", color: "var(--text-muted)", fontSize: 12 }}>
            {t("flowRunHistory.emptyState")}
          </div>
        )}
