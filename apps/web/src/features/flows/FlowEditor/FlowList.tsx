...
function formatRelativeTime(date: string): string {
  ...
  if (isNaN(d.getTime())) return i18n.t("workflows.relativeTime.unknown");
  const now = new Date();
  const diffMs = now.getTime() - d.getTime();
  const diffMin = Math.floor(diffMs / 60000);
  if (diffMin < 1) return i18n.t("workflows.relativeTime.justNow");
  if (diffMin < 60) return i18n.t("workflows.relativeTime.minutesAgo", { n: diffMin });
  const diffH = Math.floor(diffMin / 60);
  if (diffH < 24) return i18n.t("workflows.relativeTime.hoursAgo", { n: diffH });
  const diffD = Math.floor(diffH / 24);
  return i18n.t("workflows.relativeTime.daysAgo", { n: diffD });
}
...
function RunStatusBadge({ status }: { status: string | null | undefined }) {
  ...
  const config: Record<string, { bg: string; color: string; border: string; icon: typeof CheckCircle2; label: string }> = {
    completed: { bg: "var(--success-light)", color: "var(--success)", border: "#bbf7d0", icon: CheckCircle2, label: t("workflows.lastRunOk") },
    failed: { bg: "var(--danger-light)", color: "var(--danger)", border: "#fecaca", icon: XCircle, label: t("workflows.lastRunFailed") },
    running: { bg: "var(--warning-light)", color: "var(--warning)", border: "#fde68a", icon: Clock, label: t("workflows.running") },
  };
          ...
          {flows.map((flow) => (
            ...
            >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                    ...
                    >
                      ...
                      />
                      {flow.status === "enabled" ? t('workflows.enabled') : t('workflows.disabled')}
                    </span>
              ...
              <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 8 }}>
                {t('workflows.updatedTime', { time: formatRelativeTime(flow.updatedAt) })}
              </div>
            </div>
          ))}
        </div>
