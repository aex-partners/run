...
function MessageItem({ message, isLast, onDownloadAttachment }: { message: MailMessage; isLast: boolean; onDownloadAttachment?: (att: MailAttachment) => void }) {
  ...
  return (
    <div style={{
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)' }}>{message.from}</span>
            <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>&lt;{message.fromEmail}&gt;</span>
          </div>
          {collapsed ? (
            ...
            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              {t('mailDetail.toRecipients', { recipients: message.to.join(', ') })}
              {message.cc && message.cc.length > 0 && t('mailDetail.ccRecipients', { recipients: message.cc.join(', ') })}
            </span>
          )}
        </div>
      ...
      {!collapsed && (
        <>
          <div style={{ padding: '0 0 0 42px' }}>
            <EmailViewer html={message.content} />
          </div>
          {message.attachments && message.attachments.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 12, paddingLeft: 42 }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          <h2 style={{ fontSize: 22, fontWeight: 400, color: 'var(--text)', margin: 0, flex: 1, lineHeight: 1.3 }}>
            {subject}
          </h2>
          <button
            onClick={onStar}
            title={starred ? t('mail.unstar') : t('mail.star')}
            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, display: 'flex' }}
          >
            <Star size={18} fill={starred ? '#f59e0b' : 'none'} color={starred ? '#f59e0b' : 'var(--text-muted)'} />
          </button>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          {labels.map((label) => (
          ...
          <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>
            {t('mail.messagesInThread', { count: messages.length })}
          </span>
        </div>
      </div>
