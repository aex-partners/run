...
const STEP_LABEL_KEYS: Record<DiscoverStep, { key: string; fallback: string } | null> = {
  idle: null,
  checking_known_providers: { key: 'mail.setup.steps.checkingKnownProviders', fallback: 'Checking known providers...' },
  checking_autoconfig: { key: 'mail.setup.steps.checkingAutoconfig', fallback: 'Looking up autoconfig...' },
  checking_mx_records: { key: 'mail.setup.steps.resolvingMxRecords', fallback: 'Resolving MX records...' },
  trying_common_patterns: { key: 'mail.setup.steps.tryingCommonPatterns', fallback: 'Testing common server patterns...' },
  testing_connection: { key: 'mail.setup.steps.testingConnection', fallback: 'Testing connection...' },
  verifying_smtp: { key: 'mail.setup.steps.verifyingSmtp', fallback: 'Verifying SMTP credentials...' },
  verifying_imap: { key: 'mail.setup.steps.verifyingImap', fallback: 'Verifying IMAP credentials...' },
  syncing: { key: 'mail.setup.steps.syncing', fallback: 'Downloading emails...' },
  done: { key: 'mail.setup.steps.done', fallback: 'Connected!' },
  failed: { key: 'mail.setup.steps.failed', fallback: 'Could not auto-detect settings' },
}
...
export function EmailSetup({
  ...
  const stepLabel = (step: DiscoverStep) => {
    const entry = STEP_LABEL_KEYS[step]
  ...
  // Ref to track the discovery interval for cleanup on unmount
  const discoverTimerRef = useRef<ReturnType<typeof setInterval> | null>(null)
  ...
  const progressPercent = (() => {
    ...
    if (idx === -1) return 0
    return Math.round(((idx + 1) / (STEP_ORDER.length || 1)) * 100)
  })()
  ...
  const handleAutoDiscover = async () => {
    ...
    try {
      ...
      let settings: typeof _discoveredSettings = null
      ...
      if (onDiscover) {
        ...
        discoverTimerRef.current = stepTimer
      ...
      if (onVerifySmtp) {
        ...
        if (!smtpResult.ok) {
          setError(t('mail.setup.smtpError', 'SMTP: {{error}}', { error: smtpResult.error }))
          setShowManual(true)
          setDiscovering(false)
          markStep('failed')
      ...
      if (onVerifyImap) {
        ...
        if (!imapResult.ok) {
          setError(t('mail.setup.imapError', 'IMAP: {{error}}', { error: imapResult.error }))
          setShowManual(true)
          setDiscovering(false)
          markStep('failed')
          setManual({
      ...
      setError(err instanceof Error ? err.message : t('mail.setup.unknownError', 'Unknown error'))
      markStep('failed')
      setShowManual(true)
    } finally {
      setDiscovering(false)
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div>
          ...
          <Input
            placeholder={t('mail.setup.emailPlaceholder', 'you@company.com')}
            value={email}
          ...
          return (
            ...
            >
              <div style={{
                ...
                background: isCompleted ? '#dcfce7' : isCurrent ? 'var(--accent-light)' : '#f3f4f6',
    ...
    <div style={{
      ...
      {expanded && (
        <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 100px', gap: 8 }}>
            <Input
              placeholder={t('mail.setup.imapHostPlaceholder', 'imap.gmail.com')}
