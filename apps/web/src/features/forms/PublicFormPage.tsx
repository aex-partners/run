...
export function PublicFormPage() {
      ...
      <div style={containerStyle}>
        <div style={cardStyle}>
          <div style={{ color: 'var(--text-muted, #6b7280)', fontSize: 14, textAlign: 'center' }}>
            {t('database.forms.public.loading')}
          </div>
        </div>
      </div>
    )
  ...
  const toGridColumn = (ef: typeof form.entityFields[0]): GridColumn => ({
    ...
    // options are { value, label, color } objects from the API; map to the
    // select-option shape (previously this put the whole object into value/label,
    // rendering "[object Object]" in the public form's select).
  ...
  const handleSubmit = () => {
    ...
    submitMutation.mutate({ token, data: data as JsonObject })
  }
      ...
      <div style={{ ...cardStyle, maxWidth: 520, width: '100%' }}>
        ...
        <button
          ...
          style={{
            ...
            gap: 6,
            padding: '10px 24px',
            background: submitMutation.isPending ? '#9ca3af' : 'var(--accent)',
            border: 'none',
            borderRadius: 6,
            color: '#fff',
            fontSize: 14,
