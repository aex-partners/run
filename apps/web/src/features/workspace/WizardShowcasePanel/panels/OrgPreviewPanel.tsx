...
export function OrgPreviewPanel({ orgName, orgLogo }: OrgPreviewPanelProps) {
  const { t } = useTranslation()
  const displayName = orgName?.trim() || t('setup.showcase.org.defaultName')
      ...
      >
        ...
        <div style={{ padding: '20px 16px 12px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: '1px solid var(--border)' }}>
          {orgLogo ? (
            ...
            <div style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 12, fontWeight: 700 }}>
              {initials}
            </div>
          )}
