...
const DEFAULT_LINES: { icon: React.ReactNode; textKey: string }[] = [
  { icon: <Database size={13} />, textKey: 'pathPreview.default.analyzingNiche' },
  { icon: <Zap size={13} />, textKey: 'pathPreview.default.generatingRoutines' },
  { icon: <Users size={13} />, textKey: 'pathPreview.default.preparingTeamWorkspace' },
  { icon: <Shield size={13} />, textKey: 'pathPreview.default.configuringPermissionsRoles' },
  { icon: <Bot size={13} />, textKey: 'pathPreview.default.activatingAssistants' },
  { icon: <Database size={13} />, textKey: 'pathPreview.default.buildingSchemas' },
  { icon: <Zap size={13} />, textKey: 'pathPreview.default.settingUpTriggers' },
  { icon: <Users size={13} />, textKey: 'pathPreview.default.syncingTeamPreferences' },
]

const SCRATCH_LINES: { icon: React.ReactNode; textKey: string }[] = [
  { icon: <Database size={13} />, textKey: 'pathPreview.scratch.initializingWorkspace' },
  { icon: <Shield size={13} />, textKey: 'pathPreview.scratch.configuringBasePermissions' },
  { icon: <Bot size={13} />, textKey: 'pathPreview.scratch.activatingAssistant' },
  { icon: <Zap size={13} />, textKey: 'pathPreview.scratch.preparingBuilderTools' },
  { icon: <Database size={13} />, textKey: 'pathPreview.scratch.creatingBlankDatabase' },
  { icon: <Users size={13} />, textKey: 'pathPreview.scratch.settingUpAdminWorkspace' },
]
...
export function PathPreviewPanel({ selectedPath }: PathPreviewPanelProps) {
  ...
  useEffect(() => {
    ...
    setLines([{ id: 0, icon: source[0].icon, text: t(source[0].textKey), done: false }])
    idxRef.current = 1
    ...
    intervalRef.current = setInterval(() => {
      ...
      setLines((prev) => {
        ...
        const newLine = { id: idx, icon: source[nextIdx].icon, text: t(source[nextIdx].textKey), done: false }
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 420, margin: '0 auto', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        {!selectedPath && (
          <div style={{ textAlign: 'center' }}>
            ...
            <h2 style={{ margin: 0, fontSize: 24, fontWeight: 700, color: '#fff' }}>{t('pathPreview.aiPoweredSetup')}</h2>
            <p style={{ margin: '12px 0 0', fontSize: 14, color: 'rgba(255,255,255,0.65)', lineHeight: 1.5 }}>
              {t('pathPreview.chooseAPathDescription')}
            </p>
          </div>
        )}
            ...
            <div style={{ textAlign: 'center', marginBottom: 32 }}>
              <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: '#fff' }}>
                {selectedPath === 'default'
                  ? t('pathPreview.buildingYourWorkspace')
                  : t('pathPreview.preparingCleanSlate')}
              </h2>
              <p style={{ margin: '8px 0 0', fontSize: 13, color: 'rgba(255,255,255,0.6)' }}>
                {selectedPath === 'default'
                  ? t('pathPreview.defaultPathDescription')
                  : t('pathPreview.scratchPathDescription')}
              </p>
            </div>
