...
import { ChipSelect } from '../../../shared/ui/ChipSelect/ChipSelect'
import { FileUpload } from '../../files/FileUpload/FileUpload'
import { MultiSelect } from '../../../shared/ui/MultiSelect/MultiSelect'
import { TeamInviteRow } from '../../settings/TeamInviteRow/TeamInviteRow'
import { OnboardingPathCard } from '../OnboardingPathCard/OnboardingPathCard'
import { RoutineSelector } from '../RoutineSelector/RoutineSelector'
import { Button } from '../../../shared/ui/Button/Button'
import { WelcomePanel } from '../WizardShowcasePanel/panels/WelcomePanel'
import { OrgPreviewPanel } from '../WizardShowcasePanel/panels/OrgPreviewPanel'
import { NichePreviewPanel } from '../WizardShowcasePanel/panels/NichePreviewPanel'
import { LocalePreviewPanel } from '../WizardShowcasePanel/panels/LocalePreviewPanel'
import { TeamPreviewPanel } from '../WizardShowcasePanel/panels/TeamPreviewPanel'
import { PathPreviewPanel } from '../WizardShowcasePanel/panels/PathPreviewPanel'
import { RoutinesPreviewPanel } from '../WizardShowcasePanel/panels/RoutinesPreviewPanel'
import { EmailPreviewPanel } from '../WizardShowcasePanel/panels/EmailPreviewPanel'
import { PluginsPreviewPanel } from '../WizardShowcasePanel/panels/PluginsPreviewPanel'
...
import type { OnboardingPath } from '../WizardShowcasePanel/panels/PathPreviewPanel'
...
export function NewWorkspaceWizard({ onSubmit, onCreateAccount, initialStep = 0, initialData }: NewWorkspaceWizardProps) {
  ...
  useEffect(() => {
    ...
    // Only run once on mount; initialData is intentionally excluded to avoid re-triggering
  ...
  const handleNext = useCallback(async () => {
    ...
    if (step === 0 && !accountCreated && onCreateAccount) {
      try {
        ...
        setErrors({ email: err instanceof Error ? err.message : t('setup.account.createAccountFailed') })
        return
      } finally {
        setSubmitting(false)
      }
  ...
  const showcaseContent = useMemo(() => {
    switch (step) {
      case 0:
        return <WelcomePanel />
      case 1:
        return <OrgPreviewPanel orgName={data.orgName} orgLogo={data.orgLogo} />
      case 2:
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 340, overflowY: 'auto' }}>
        ...
        {catalog.length === 0 && (
          <div style={{ padding: 20, textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>
            {t('setup.plugins.loadingCatalog')}
          </div>
        )}
      </div>
    </div>
