    ...
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div>
        ...
        <Input
          ...
          placeholder="e.g. get_orders (snake_case)"
        />
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <label style={{ fontSize: 13, color: 'var(--text)' }}>Read-only (safe to run without confirmation)</label>
        <Toggle
      ...
      {onTest && (
        <div style={{ background: 'var(--surface-2)', borderRadius: 8, padding: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: testResult ? 10 : 0 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>Test Panel</span>
