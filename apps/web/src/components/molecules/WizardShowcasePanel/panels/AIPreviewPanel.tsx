...
function QualityBar({ percent, label }: { percent: number; label: string }) {
    ...
    <div style={{ padding: '10px 14px', background: '#fff', borderRadius: 10, border: '1px solid var(--border)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 500 }}>{t.setup.showcase.ai.quality}</span>
        </div>
      ...
      <div style={{ width: '100%', maxWidth: 340, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24, textAlign: 'center' }}>
        ...
        {provider === 'openai' && (
          <>
            ...
            <QualityBar percent={100} label="100%" />
          </>
        )}
        ...
        {provider === 'ollama' && spec && (
          <>
            ...
            <QualityBar percent={spec.quality} label={spec.qualityLabel} />
          </>
        )}
      </div>
    </div>
