...
const setup = (byUser: Record<string, string[]>) => {
  ...
  const service = new SetReadStateService(accounts, emails, events, fixedClock(NOW))
  return { accounts, emails, events, service }
}

describe('SetReadStateService', () => {
  it('marks accessible unread emails read, saving each and publishing EmailReadChanged', async () => {
    const { emails, events, service } = setup({ user: ['acc-1'] })
    emails.seed(seedEmail({ id: 'e1', accountId: 'acc-1', read: false }))
    emails.seed(seedEmail({ id: 'e2', accountId: 'acc-1', read: false }))

    const res = await service.execute({ actorId: 'user', ids: ['e1', 'e2'], read: true })

    expect(res.ok).toBe(true)
    expect(emails.store.get('e1')!.read).toBe(true)
    expect(emails.store.get('e2')!.read).toBe(true)
    expect(events.events.map((e) => e.name)).toEqual(['email.EmailReadChanged', 'email.EmailReadChanged'])
  })

  it('returns success without work when the user owns no accounts', async () => {
    const { emails, events, service } = setup({})
    emails.seed(seedEmail({ id: 'e1', accountId: 'acc-1' }))

    const res = await service.execute({ actorId: 'nobody', ids: ['e1'], read: true })
  ...
  it('emits no event when the email is already in the requested read state (idempotent setRead)', async () => {
    const { emails, events, service } = setup({ user: ['acc-1'] })
    emails.seed(seedEmail({ id: 'e1', accountId: 'acc-1', read: true }))

    const res = await service.execute({ actorId: 'user', ids: ['e1'], read: true })

    expect(res.ok).toBe(true)
    // The service still saves the (unchanged) aggregate, but no event is recorded.
    expect(emails.saved).toHaveLength(1)
    expect(events.events).toHaveLength(0)
  })

  it('marks a read email unread and publishes the change', async () => {
    const { emails, events, service } = setup({ user: ['acc-1'] })
    emails.seed(seedEmail({ id: 'e1', accountId: 'acc-1', read: true }))

    const res = await service.execute({ actorId: 'user', ids: ['e1'], read: false })

    expect(res.ok).toBe(true)
    expect(emails.store.get('e1')!.read).toBe(false)
    expect(events.events.map((e) => e.name)).toEqual(['email.EmailReadChanged'])
  })
})
