...
import { PinMessage, PinMessageCommand } from '@/contexts/conversations/application/ports/in/PinMessage'
...
// Toggles a message's pinned flag and broadcasts the update to members.
export class PinMessageService implements PinMessage {
  constructor(
  ...
  async execute(cmd: PinMessageCommand): Promise<Result<{ success: true; pinned: boolean }>> {
    ...
    const pinned = message.togglePin(memberIds, this.clock.now())

    await this.messages.save(message)
    await this.events.publish(message.pullEvents())

    return ok({ success: true, pinned })
  }
}
