...
const setup = () => {
  ...
  const service = new RenameFileService(files, events, fixedClock(NOW))
...
describe('RenameFileService', () => {
  it('fails when the file does not exist', async () => {
    ...
    const res = await service.execute({ id: 'missing', name: 'b.txt' })
    expect(res.ok).toBe(false)
    expect(files.saved).toHaveLength(0)
    expect(events.events).toHaveLength(0)
  })

  it('renames the file, saves and publishes FileRenamed', async () => {
    const { files, events, service } = setup()
    const f = seedFile(files, 'f1')
    const res = await service.execute({ id: 'f1', name: 'renamed.txt' })
    expect(res.ok).toBe(true)
    expect(f.name).toBe('renamed.txt')
    expect(files.saved).toContain(f)
    expect(events.events.map((e) => e.name)).toEqual(['files.FileRenamed'])
  })

  it('enforces the non-empty name guard, does not save and publishes nothing', async () => {
    const { files, events, service } = setup()
    const f = seedFile(files, 'f1')
    const res = await service.execute({ id: 'f1', name: '   ' })
    expect(res.ok).toBe(false)
