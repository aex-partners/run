// Business-process skill templates, ported 1:1 from the source
// `ai/skill-templates.ts`. PURE domain data + a pure selector: each template maps
// to a category of routines selectable in the setup wizard; when a user selects
// routines, the matching category templates are auto-linked to the assistant.
// No IO, no npm — the composition root reads these to seed `skills` rows.
...
export const SKILL_TEMPLATES: SkillTemplate[] = [
  // ─── SALES & CRM ────────────────────────────────────────────
  {
    ...
    name: 'Sales & CRM',
    description:
      'Manages leads, sales pipeline, proposals, customer relationships, commissions, and order processing.',
    category: 'sales',
    routineIds: [
      'lead-capture',
      'sales-pipeline',
...
- NF-e number must be recorded on receiving`,
  ...
  // ─── INVENTORY & STOCK ───────────────────────────────────────
  {
    ...
    description:
      'Manages products, stock levels, movements, warehouse locations, and inventory audits.',
    category: 'inventory',
...
- EAN/GTIN barcode standards`,
  ...
  // ─── FINANCIAL ───────────────────────────────────────────────
  {
    ...
    description:
      'Manages accounts payable/receivable, cash flow, expenses, budgets, and tax obligations.',
    category: 'finance',
    routineIds: [
      'accounts-receivable',
      'accounts-payable',
...
- SPED fiscal compliance data`,
  ...
  // ─── INVOICING / NF-e ───────────────────────────────────────
  {
    ...
    name: 'Invoicing & NF-e',
    description:
      'Manages Brazilian electronic invoices (NF-e, NFS-e, NFC-e), tax calculations, and fiscal compliance.',
    category: 'finance',
...
- Insalubridade/periculosidade: 10-40% additional`,
  ...
  // ─── MANUFACTURING / PRODUCTION ──────────────────────────────
  {
    ...
    description:
      'Manages bills of materials, production orders, scheduling, quality control, and maintenance.',
    category: 'operations',
...
- Tabela de frete minimo (minimum freight table)`,
  ...
  // ─── MARKETING ───────────────────────────────────────────────
  {
    ...
    name: 'Marketing',
    description:
      'Manages campaigns, lead generation, analytics, content calendar, and marketing spend.',
    category: 'sales',
...
- Livro Diario and Livro Razao requirements`,
  ...
  // ─── E-COMMERCE ──────────────────────────────────────────────
  {
    ...
    name: 'E-commerce',
    description:
      'Manages online store operations, product listings, online orders, and marketplace integrations.',
    category: 'sales',
...
// PURE selector. Given the routine IDs chosen at setup, returns the matching
// templates (a template matches if ANY of its routineIds was selected).
export function getSkillsForRoutines(routineIds: string[]): SkillTemplate[] {
  const routineSet = new Set(routineIds)
  return SKILL_TEMPLATES.filter((skill) => skill.routineIds.some((rid) => routineSet.has(rid)))
}
