...
export const definirOperacaoTool = (uc: DefinirOperacao): ToolDefinition => ({
  ...
  readOnly: false,
  description:
    'Cria ou atualiza uma operação do roteiro de produção de um Modelo (rascunho). Input: { id?: string, modeloId: string, codigo: string, seq: number, nome: string, centroId: string | null, tempoPadraoMin: number, tempoPorTamanho?: { [tamanho: string]: number }, tempoSetupMin?: number, loteSetup?: number, agregada?: boolean }. Sem id cria; com id atualiza (SÓ rascunho: operação já publicada é imutável — chame abrir_revisao_roteiro antes e edite o rascunho clonado). codigo = identidade ESTÁVEL da operação no modelo (ex.: CORTE, COSTURA, ACABAMENTO), preservada em todas as revisões; é por ele que a linha da ficha técnica (operacao_codigo) diz qual operação consome cada insumo — por isso o codigo é IMUTÁVEL no update (mandar outro codigo com o mesmo id FALHA; para uma operação diferente, crie outra linha sem id). Para ADICIONAR uma operação a um roteiro JÁ PUBLICADO, chame abrir_revisao_roteiro ANTES: criar só o rascunho novo e publicar deixaria a revisão com apenas ele (publicar_roteiro recusa, nomeando as operações que faltam). Todos os tempos em MINUTOS. tempoPorTamanho sobrepõe tempoPadraoMin para os tamanhos informados. A operação fica em RASCUNHO e só passa a custear depois de publicar_roteiro. Retorna { id }.',
