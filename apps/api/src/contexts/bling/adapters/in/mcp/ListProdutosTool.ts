...
// produtos from the connected Bling account. Requires Bling to be connected in
// Settings; the use-case surfaces that failure verbatim.
export const listProdutosTool = (uc: ListBlingResource): ToolDefinition => ({
  name: 'bling_listar_produtos',
  readOnly: true,
  description:
    'Lista produtos do Bling (ERP). Input: { pagina?: number, limite?: number, pesquisa?: string }. Retorna { items: [...] } com os produtos. Requer o Bling conectado em Settings.',
  async execute(input: Json) {
    const parsed = parseBlingInput(input)
    if (!parsed) return fail('bling_listar_produtos: expected an object')
