...
describe('DataRecordSink', () => {
  it('inserts when unseen, skips when hash unchanged, updates when changed', async () => {
    ...
    const insert = { execute: async () => { inserts++; return ok({ id: 'r1', version: 0 }) } }
    ...
    const sink = new DataRecordSink({ insert, update, get, syncMap, clock } as never)

    const a = await sink.upsertExternal({ entityId: 'e1', slug: 'bling_depositos', externalId: '1', data: { descricao: 'x' } })
    expect(a.ok && a.value.inserted).toBe(true); expect(inserts).toBe(1)
    const b = await sink.upsertExternal({ entityId: 'e1', slug: 'bling_depositos', externalId: '1', data: { descricao: 'x' } })
    expect(b.ok && b.value.changed).toBe(false); expect(updates).toBe(0) // unchanged → skip
    const c = await sink.upsertExternal({ entityId: 'e1', slug: 'bling_depositos', externalId: '1', data: { descricao: 'y' } })
