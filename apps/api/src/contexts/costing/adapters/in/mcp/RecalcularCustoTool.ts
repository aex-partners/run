...
export const recalcularCustoTool = (uc: RecalcularCusto): ToolDefinition => ({
  ...
  readOnly: false,
  description:
    'Recalcula o custo re-explodindo a(s) ficha(s): informe skuId para um SKU específico, ou modeloId para todos os SKUs de um modelo. Input: { skuId?: string, modeloId?: string }. Retorna { recalculados }.',
  async execute(input: Json) {
    ...
    const r = await uc.execute({ skuId: skuId.value, modeloId: modeloId.value })
