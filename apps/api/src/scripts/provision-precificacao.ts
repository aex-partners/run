// Idempotente: cria as 4 entidades de precificação + acrescenta desp_financeira a
// condicoes_pagamento, pelas in-ports do contexto `data`. Seguro re-rodar. Rodar com
// DATABASE_URL. `condicoes_pagamento`, `modelos`, `produtos` já existem (espelho do Bling).
...
async function main() {
  ...
  for (const dep of ['modelos', 'produtos', 'condicoes_pagamento']) {
    if (!(await idBySlug(dep))) throw new Error(`entidade ${dep} não existe: rode o seed do Bling antes`)
  }

  // 1) cria as entidades sem campos (idempotente por slug), para que as relações
  //    entre elas (politica_de_preco.canal, precos_de_venda.canal) resolvam no passo 2.
  for (const spec of PRECIFICACAO_ENTITIES) {
  ...
  for (const spec of PRECIFICACAO_ENTITIES) await addFieldsTo(spec.slug, spec.fields)
  await addFieldsTo('condicoes_pagamento', CONDICOES_PRECO_FIELDS)

  console.log('provision-precificacao done')
  process.exit(0)
}
