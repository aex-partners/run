import { I18nForPiece, PieceMetadataModel, PieceMetadataModelSummary } from "./piece-metadata.js"
import { LocalesEnum, MAX_KEY_LENGTH_FOR_CORWDIN } from "./compat/shared-types.js"
...
function translateProperty(pieceModelOrProperty: Record<string, unknown>, path: string, i18n: Record<string, string>) {
  ...
  const nextObject = pieceModelOrProperty[head] as Record<string, unknown>;
  if (!nextObject) {
    return;
  }
  ...
  const propertyValue = pieceModelOrProperty[head] as string
  const valueInI18n = i18n[propertyValue.slice(0, MAX_KEY_LENGTH_FOR_CORWDIN)]
  if (valueInI18n) {
    pieceModelOrProperty[head] = valueInI18n
  }
}
